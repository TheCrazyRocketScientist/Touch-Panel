...
if __name__ == "__main__":
    ...
    train_data,validation_data,test_data,train_labels,validation_labels,test_labels = get_train_data(window_size=0.6,overlap=0.1)
    X_test = np.concatenate((validation_data, test_data), axis=0)
    y_test = np.concatenate((validation_labels, test_labels), axis=0)

    #remove noise from the datasetss
    train_noise = train_data[train_labels == 0]
    X_train = train_data[train_labels != 0]
    y_train = train_labels[train_labels != 0]


    #get sequence lenght from shape of data
    sequence_length = X_train.shape[1]

    #get test train split [removed]
