import os
import csv
import smbus2
import argparse
import RPi.GPIO as GPIO
from pi.ADXL import ADXL345
from aiocsv import AsyncWriter
from datetime import datetime
import asyncio
import aiocsv

parser = argparse.ArgumentParser()

parser.add_argument("-n","--sensor_num",type=int,help="Sets SPI Device Number",default=0)
parser.add_argument("-p","--int_pin",type=int,help="Sets interrupt pin for ADXL",default=4)
parser.add_argument("-b","--bus",type=int,help="Sets up bus number",default=0)
parser.add_argument("-a", "--alt_addr", action="store_true",help="Use the alternate ADXL address (0x1D). Default is 0x53.") 
parser.add_argument("-s","--batch_size",type=int,help="Sets batch size for writing data to disk",default=100)

args = parser.parse_args()

addr_sel =args.alt_addr
sensor_num = args.sensor_num
batch_size = args.batch_size
int_pin = args.int_pin
bus = args.bus

bus_obj = smbus2.SMBus(bus)

GPIO.setmode(GPIO.BCM)
GPIO.setup(int_pin, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)

sensor = ADXL345("sensor0", bus_obj, int_pin, address_select=addr_sel, number=0)

sensor.startup()
sensor.get_data()

file_name = f"SENSOR{sensor_num}_data.csv"

buffer = []
buffer_lock = asyncio.Lock()


async def insert_data():

    global buffer
    
    while True:
        
        async with buffer_lock:

            if(len(buffer)) >= batch_size:

                file_buffer = buffer
                buffer = []

                async with aiofiles.open(file_name,mode="a",newline="") as my_file:

                    writer = AsyncWriter(my_file, dialect="excel")
                    await writer.writerows(file_buffer)

                    await my_file.flush()


        await asyncio.sleep(0.001)


def read_sensor(readings):

    global buffer
    current_time = datetime.now()
    formatted_time = current_time.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]

    #event_loop.call_soon_threadsafe(buffer.append,[formatted_time,sensor.x,sensor.y,sensor.z])

    buffer.append([formatted_time]+readings)


async def main():

    tasks = [insert_data()]
    await asyncio.gather(*tasks)

if __name__ == "__main__":

    try:

        event_loop = asyncio.get_event_loop()

        if not os.path.exists(file_name) or os.stat(file_name).st_size == 0:
            with open(file_name, mode="w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["timestamp", "x", "y", "z"])

        sensor.register_callback(read_sensor)
        sensor.startup()
        sensor.get_data()
        asyncio.run(main())

    except KeyboardInterrupt:

        print("Interrupted, flushing buffer to disk...")
        if buffer:
            with open(file_name, mode="a", newline="") as f:
                writer = csv.writer(f)
                writer.writerows(buffer)
        sensor.close()
        print("Sensor closed. Exiting.")

         